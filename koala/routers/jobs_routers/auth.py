...
async def login(
    is_company_user: bool = False, credentials: OAuth2PasswordRequestForm = Depends()
):
    try:
        user, scopes = await authentication.authenticate(credentials)
        logging.info(user)
        logging.info(scopes)
        if not user:
            raise HTTPException(
        ...
        access_token = create_access_token(
            data={"username": user.username, "email": user.email, "scopes": scopes},
